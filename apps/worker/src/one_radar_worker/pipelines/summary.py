...
def _summary_prompt(*, title: str, source_text: str, content_type: str = "") -> str:
    if content_type in {"podcast_episode", "bilibili_video"}:
        instruction = (
            ...
            "2. 不要使用 Markdown 标题符号，例如 #、##、###、####。分区标题只用加粗文本，例如 **总览**。\n"
            ...
            "4. 如果是行业分析、知识讲解或观点讨论，使用这些分区：**总览**、**核心结论**、**关键事实与证据**、**行动或追踪问题**。这些分区下用项目符号，格式为：- **短标签**：具体内容。\n"
        ...
        instruction = (
            ...
            "1. 不要编造原文没有的信息。\n"
            "2. 不要使用 Markdown 标题符号，例如 #、##、###、####。分区标题只用加粗文本，例如 **总览**。\n"
            "3. 输出 **总览**、**核心要点**、**值得保存** 三个分区；要点用项目符号，格式为：- **短标签**：具体内容。\n"
            "4. 不输出寒暄，不使用表格。\n"
        )
    return (
        "你是 OneRadar 的知识整理助手。"
