...
def _summary_prompt(*, title: str, source_text: str, content_type: str = "") -> str:
    if content_type in {"podcast_episode", "bilibili_video"}:
        ...
        instruction = (
            ...
            "1. 不要编造原文没有的信息。\n"
            "2. 使用 Markdown 格式组织内容。分区标题使用二级标题，例如 ## 总览；要点使用项目符号，格式为：- **短标签**：具体内容。\n"
            "3. 输出 ## 总览、## 核心要点、## 可操作信息、## 值得保存 四个分区。\n"
            "4. ## 总览 用 1-2 个自然段讲清主题、背景、核心结论和为什么重要。\n"
            "5. ## 核心要点 保留主要观点、关键事实、数字、对比、因果关系和限制条件，不要只写抽象结论。\n"
            "6. ## 可操作信息 专门整理原文中可复用的方法、流程、配置、接口、工具、资源、决策步骤或检查清单；如果原文没有这类信息，写“原文没有明确的可操作步骤”。\n"
            "7. ## 值得保存 提炼后续检索时最有价值的细节，包括名称、数据、链接线索、适用场景、风险、争议、待跟进问题或原文中容易被摘要遗漏的具体信息。\n"
            "8. 摘要可以比普通短摘要更充分，优先完整保留有复用价值的信息；不要为了简短而省略原文中的步骤、参数、示例、限制和数字。\n"
            "9. 不输出寒暄，不使用表格。\n"
        )
    return (
        "你是 OneRadar 的知识整理助手。"
