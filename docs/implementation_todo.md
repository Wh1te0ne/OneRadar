...
HTTP contract coverage now exists for GET /api/feeds/preview, POST /api/items/import, single-user workspace bootstrap, and the absence of public login/logout routes, including fallback-store dedup behavior, but broader parser/E2E coverage is still pending.
