...
OneRadar now also exposes a built-in `/api/mcp` JSON-RPC endpoint for Hermes Agent. The MCP tools list RSS source state, return structured entries for a requested time window, and support `refresh_news` so external callers can trigger current-user RSS refresh, wait for translation completion, and then read fresh window data in one call. This MCP path is intentionally hosted inside the existing API container, not a separate Docker service, so PostgreSQL remains the single news-source state owner and Hermes remains responsible for AI grouping and delivery.
