...
The 每日新闻 page is the first formal RSS homepage: it reads cached RSS entries for a selected date, calls the configured summarization/chat model to translate and summarize them into a fixed daily-brief structure, persists one report per user per date, and links section entries back into the source-level Feed page or transient article preview. The API schedules default generation at 10:00 Asia/Shanghai and the desktop UI supports previous/next date browsing, date picking, missing-date generation, and confirmed regeneration that overwrites the existing report for that day. It intentionally avoids automatic item import and independent RSS display areas.
